...
public class StudentRegisterApp {
	...
	private static void procUpdate() {
	...
	private static void procUpdateDetail(String sId, String sPw) {
		Student student = dao.readUserFromId(sId);
		
		//System.out.println(student);
		System.out.println("변경하고자 하는 항목에 입력");
		...
		while (true) {
			...
			if (pw1.equals(pw2) && pw1.length() != 0) {
				...
				System.out.println("비밀번호 확인하고 다시 입력");
			}
		}
		
		while (true) {
			try {
				System.out.print("전공(" + student.getsMajor() + "): ");
				String strMajor = scan.nextLine().split("\\s+")[0];
				...
				System.out.println("전공입력이 잘못, 다시입력(정수)");
			}
		}
		
		while (true) {
			System.out.print("생년월일(" + student.getsBirthDateToStr() + "): ");
			String strBirthDay = scan.nextLine().split("\\s+")[0];
			
			if (strBirthDay.length() != 0) {
				String[] tokens = strBirthDay.split("-");
				if (tokens.length == 3) {
					student.setsBirthDate(strBirthDay);
					break;
				} else {
					System.out.println("잘못된 포맷, 다시입력");
				}
			} else {
				break;
			}
